...
class HelpSession:
    ...
    def _get_query(self, query: str) -> Union[Command, Cog]:
        ...
        if cog_matches:
            ...
            cmds = (cog.get_commands()
                    for cog in cog_matches)  # Commands of all cogs

            return Cog(
                name=cog.category if hasattr(
                    cog, "category") else cog.qualified_name,
    ...
    def _handle_not_found(self, query: str) -> None:
        ...
        result = process.extractBests(
            query, choices, scorer=fuzz.ratio, score_cutoff=90)
    ...
    async def build_pages(self) -> None:
        ...
        paginator = LinePaginator(
            prefix='', suffix='', max_lines=self._max_lines)
        ...
        # save organized pages to session
        self._pages = paginator.pages
    ...
    def embed_page(self, page_number: int = 0) -> Embed:
        ...
        if page_count > 1:
            embed.set_footer(
                text=f'Page {self._current_page+1} / {page_count}')
...
class Help(DiscordCog):
    ...
    @redirect_output(destination_channel=Channels.commands, bypass_roles=STAFF_ROLES)
