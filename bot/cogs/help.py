...
class CustomHelpCommand(HelpCommand):
    ...
    async def get_all_help_choices(self) -> set:
        ...
        # all category names
        choices.update(cog.category for cog in self.context.bot.cogs.values(
    ...
    async def command_formatting(self, command: Command) -> Embed:
        ...
        embed.set_author(name="Command Help",
                         icon_url=constants.Icons.questionmark)
        ...
        aliases = ", ".join(
            f"`{alias}`" if not parent else f"`{parent} {alias}`" for alias in command.aliases)
        if aliases:
