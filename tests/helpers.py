...
class MockUser(CustomMockMixin, unittest.mock.Mock, ColourMixin, HashableMixin):
    ...
    def __init__(self, **kwargs) -> None:
        default_kwargs = {"name": "user", "id": next(
            self.discord_id), "bot": False}
        super().__init__(**collections.ChainMap(kwargs, default_kwargs))
...
class MockTextChannel(CustomMockMixin, unittest.mock.Mock, HashableMixin):
    ...
    def __init__(self, **kwargs) -> None:
        default_kwargs = {"id": next(
            self.discord_id), "name": "channel", "guild": MockGuild()}
        super().__init__(**collections.ChainMap(kwargs, default_kwargs))
...
dm_channel_instance = discord.DMChannel(
    me=me, state=state, data=dm_channel_data)
...
class MockDMChannel(CustomMockMixin, unittest.mock.Mock, HashableMixin):
    ...
    def __init__(self, **kwargs) -> None:
        default_kwargs = {"id": next(
            self.discord_id), "recipient": MockUser(), "me": MockUser()}
        super().__init__(**collections.ChainMap(kwargs, default_kwargs))
...
emoji_data = {"require_colons": True,
              "managed": True, "id": 1, "name": "notimplementedemoji"}
...
reaction_instance = discord.Reaction(message=MockMessage(), data={
                                     "me": True}, emoji=MockEmoji())
